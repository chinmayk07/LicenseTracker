...
- (NSDictionary *) listDetails
{
    ...
    NSLog(@"finalDictonaryLISTDETAILS = %@",arrayOfPerson);
    return test1;
}
...
-(NSDictionary *) weekLogChart:(NSString *)firstDayOfTheWeek lastDayOfWeek:(NSString *)lastDayOfWeek {
    ...
    if(sqlite3_open([dbPathString UTF8String], &personDB)== SQLITE_OK) {
        NSString *querySql = [NSString stringWithFormat:@"Select count(LOG_TIME),strftime('%%d/%%m',LOG_TIME) from LICENSELOG where LOG_TIME BETWEEN '%@' AND '%@' group by strftime('%%d %%m %%Y',LOG_TIME)",firstDayOfTheWeek,lastDayOfWeek];
        const char* query_sql = [querySql UTF8String];
...
- (NSDictionary *) monthLogChart:(NSString *)firstDayOfTheMonth lastDayOfMonth:(NSString *)lastDayOfMonth {
    ...
    if(sqlite3_open([dbPathString UTF8String], &personDB)== SQLITE_OK) {
        NSString *querySql = [NSString stringWithFormat:@"Select count(LOG_TIME),strftime('%%d/%%m',LOG_TIME)  from LICENSELOG where LOG_TIME BETWEEN '%@' AND '%@' group by strftime('%%d %%m %%Y',LOG_TIME)", firstDayOfTheMonth,lastDayOfMonth ];
        const char* query_sql = [querySql UTF8String];
...
- (NSDictionary *)yearlyLogChart:(NSString *)currentYear {
    ...
    if(sqlite3_open([dbPathString UTF8String], &personDB)== SQLITE_OK) {
        NSString *querySql = [NSString stringWithFormat:@"Select count(LOG_TIME),strftime('%%m/%%Y',LOG_TIME) from LICENSELOG where strftime('%%Y',LOG_TIME)='%@' group by strftime('%%m %%Y',LOG_TIME)",currentYear];
        const char* query_sql = [querySql UTF8String];
